...
import cn.bywin.business.common.base.Param;
...
public class SqlUtil {
    ...
    public static void main(String[] args) {
        String sql1 = "SELECT a,{column} as tt FROM  {tableA} as ta LEFT JOIN (\n" +
...
//
        String sqlCommonStr = "select p.*, u.name as salt_key, c.content_id as content_id, c.event_create_time as event_create_time  " +
                " from process_apply p, sys_users u , process_approval a, proceintss_apply_content c " +
        ...
        String sql = " select b.approval_type as name, b.approval_type as key , count(1) as value " +
                " from (" + sqlCommonStr + ") b " +
                " group by b.approval_type order by value desc limit 5 ";

   /*     String sql = " select date_format(statistics_date, '{format}') as `key`, date_format(statistics_date, '{format}') as `name`," +
        ...
        System.out.println(parserSQL(sql1, map));
    }
}
