...
public class FlinkWhereCondParse {
    protected final Logger logger = LoggerFactory.getLogger(this.getClass());
    String errMsg;
    String debugMsg;
    String whereSql;
    List<String> useFieldList = new ArrayList<>();
    ...
    public List<String> getUseFieldList() {
        return Lists.newArrayList( useFieldList );
    }
...
//    }
    ...
    public boolean paraseComCond(String whereCond, List<TTruModelElementDo> entList, List<TTruModelFieldDo> colList) {
        errMsg = "";
        debugMsg = "";
        whereSql = "";
        ...
        try {
            if (StringUtils.isBlank(whereCond))
                return true;
            Map<String, TTruModelFieldDo> colMap = colList.stream().collect(Collectors.toMap(x -> x.getId(), y -> y));
            Map<String, TTruModelFieldDo> idFieldMap = colList.stream().collect(Collectors.toMap(x -> x.getFieldAlias(), y -> y));

            Map<String, TTruModelElementDo> entMap = entList.stream().collect(Collectors.toMap(x -> x.getId(), y -> y));
            DatasetWhereCond cond = JsonUtil.deserialize(whereCond, DatasetWhereCond.class);
            StringBuilder sb = new StringBuilder();
            if (cond == null || cond.getConditions() == null || cond.getConditions().size() == 0) {
                return true;
    ...
    private boolean dealComCond(DatasetWhereCond pc, List<DatasetWhereCond> subCond, Map<String, TTruModelElementDo> entMap,
                                Map<String, TTruModelFieldDo> colMap,Map<String, TTruModelFieldDo> idFieldMap, StringBuilder sb) throws Exception {
        ...
        for (DatasetWhereCond cond : subCond) {
            ...
            TTruModelFieldDo columnDo1 = colMap.get(cond.getFieldId1());
            if (columnDo1 == null) {
                setError(String.format("%s(%s)对应字段不存在", cond.getField1(), cond.getFieldId1()), cond.impInfo(""));
                return false;
            }
            TTruModelElementDo entityDo = entMap.get(columnDo1.getElementId());

            if (entityDo == null) {
                setError(String.format("%s(%s)对应表不存在", cond.getField1(), cond.getFieldId1()), cond.impInfo(""));
                return false;
            }
            String field =getDataFieldName( columnDo1, idFieldMap);
            if( useFieldList.indexOf( field )<0) {
                useFieldList.add(field);
            }
            sb.append(" ")
                    .append(field).append(" ");
            ...
            if (StringUtils.isNotBlank(cond.getFieldId2())) {
                TTruModelFieldDo columnDo2 = colMap.get(cond.getFieldId1());
                if (columnDo1 == null) {
                    setError(String.format("%s(%s)对应字段不存在", cond.getField2(), cond.getFieldId2()), cond.impInfo(""));
                    return false;
                }
...
//        return pre;
//    }
    public static String getDataFieldName( TTruModelFieldDo fieldDo,Map<String, TTruModelFieldDo> idFieldMap) {
...
//            }
        ...
        if (StringUtils.isNotBlank(fieldDo.getOrderFunc())) {
            ...
            for (String str : strs) {
                TTruModelFieldDo fieldDos = idFieldMap.get(str);
                if (fieldDos != null) {
                    String outNames = "";
    ...
    public static String getDataFieldName( TTruModelFieldDo fieldDo,String tableAliasPrex,
                                           Map<String, TTruModelFieldDo> idFieldMap) {
        String fieldName = fieldDo.getFieldName();
