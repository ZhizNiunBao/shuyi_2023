...
@CrossOrigin(value = {"*"},
        ...
        maxAge = 3600)
@RestController
@Api(tags = "联邦学习-字段管理-bydbfield")
@RequestMapping("/bydbfield")
public class BydbFieldController extends BaseController {
    ...
    @ApiOperation(value = "修改bydb字段", notes = "修改bydb字段")
    @ApiImplicitParams({
            //@ApiImplicitParam(name = "modelVo", value = "bydb字段", dataType = "TBydbFieldDo", required = true, paramType = "body")
    })
...
//            }
            ...
            TBydbObjectDo tmpObj = new TBydbObjectDo();
    ...
    @ApiOperation(value = "批量启用或禁用bydb字段内容", notes = "修改bydb字段内容")
    @ApiImplicitParams({
    ...
    public Object enableData(String id, Integer enable, HttpServletRequest request) {
        ...
        try {
            ...
            Example exp = new Example(TBydbFieldDo.class);
            ...
            String act = "修改-" + actType + "bydb字段" + System.currentTimeMillis();
            if (enable == 1) {
                criteria.andCondition(" (enable =0  or enable is null )");
            } else {
                criteria.andEqualTo("enable", 1);
            }
            List<TBydbFieldDo> chgList = fieldService.findByExample(exp);
            if (chgList.size() != split.size()) {
                return resMap.setErr(" 数据已变化不能操作").getResultMap();
            }
            List<TBydbFieldDo> oldList = new ArrayList<>();

            List<TBydbDatabaseDo> dbList = null;
            List<TBydbSchemaDo> schemaList = null;
            List<TBydbObjectDo> objectList = new ArrayList<>();
            if (enable == 1) {
                for (TBydbFieldDo info : chgList) {
            ...
            criteria.andIn("id", idList);
            //criteria.andCondition(" (enable =0  or enable is null )");
            List<TBydbObjectDo> objTmpList = objectService.findByExample( exp );
            for (TBydbObjectDo tmp : objTmpList) { //只更新指定字段
...
//
//                if (objectList != null) {
//                    for (TBydbObjectDo objectDo : objectList) {
...
//            }
    ...
    @ApiOperation(value = "bydb字段内容", notes = "bydb字段内容")
    @ApiImplicitParams({
    ...
    @ApiOperation(value = "删除bydb字段", notes = "删除bydb字段")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "bydb字段 id", dataType = "String", required = true, paramType = "query")
    })
...
//            }
            ...
            List<TBydbFieldDo> list = fieldService.findByExample(exp);
            int cnt = list.size();
            if (cnt == 0) {
            ...
            List<TBydbObjectDo> objList = new ArrayList<>();
...
//                try {
//                    new LogActionOp(SysParamSetOp.readValue(Constants.syspara_SystemCode, ""), HttpRequestUtil.getAllIp(request)).delLog(user, info, "删除-bydb字段" + times);
//                } catch (Exception e1) {
//                    resMap.setErr("删除失败");
...
//
//                TBydbModelFieldDo fieldDo = bydbModelFieldService.findById(id);
//                TBydbModelElementDo elementDos = bydbModelService.findById(fieldDo.getElementId());
//                List<TBydbModelFieldDo> fieldDos = new ArrayList<>();
//                if (elementDos != null) {
//                    List<TBydbModelFieldDo> fieldDos1 = bydbModelFieldService.selectByElementId(elementDos.getId());
//                    TBydbModelComponentDo componentDo = bydbModelComponentService.findById(elementDos.getTcId());
...
//            } else {
//                List<TBydbFieldDo> list = fieldService.findBeanList(modelVo);
//                //List<TBydbModelFieldDo> list1 =  bydbModelFieldService.selectByModelId();
//                resMap.setPageInfo(modelVo.getPageSize(), modelVo.getCurrentPage());
