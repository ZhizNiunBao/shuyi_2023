...
@Data
public class OlkDataSourceOutPutComponent extends OlkBaseComponenT {
    ...
    public OlkDataSourceOutPutComponent() {
    }
    ...
    public List<TOlkModelFieldDo> getShowField( List<TOlkModelFieldDo> fieldDos ) throws Exception {
        ...
        return fieldDos.stream().filter( c -> c.getIsSelect() == 1 ).
                sorted( Comparator.comparing( TOlkModelFieldDo::getFilterSort ) ).collect( Collectors.toList() );
    }
    ...
    public void build( OlkNode truNode, TOlkModelElementDo fModelElementDo, TOlkModelDo model ) throws Exception {
        if ( !"tee".equalsIgnoreCase( model.getConfig() ) && !"flink".equalsIgnoreCase( model.getConfig() ) ) {
            ...
            TOlkModelElementDo preElem = getPreModel().get( 0 );
